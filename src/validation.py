...
# =============================================================================
# VEITCH IMPROVEMENT 3: CAUSAL SUFFICIENCY TEST
# =============================================================================
...
def test_causal_sufficiency(
    ...
    """Test whether embeddings are causally sufficient (Veitch et al., 2020).

    If embedding Z is causally sufficient, then conditioning on additional
    covariates X should not change the ATE estimate. Formally:

        ATE(Z) ≈ ATE(Z, X) implies Z is approximately causally sufficient.

    A large delta suggests the embedding fails to capture all confounders,
    meaning the DML cross-fitting is compensating for missing information.

    Args:
        Y: Outcome array (n,).
        T: Treatment array (n,).
        X_embed: Embedding-based covariates (n, d_embed).
        X_raw: Raw covariates (n, d_raw) — e.g., education, ability.
        true_ate: True ATE for reference.

    Returns:
        Dictionary with ATE estimates, delta, and sufficiency assessment.
    """
    from src.dml import CausalDMLPipeline

    # ATE using embeddings only
    pipeline_z = CausalDMLPipeline()
    ate_z, _, _ = pipeline_z.fit_predict(Y, T, X_embed)
    se_z = pipeline_z.ate_se

    # ATE using embeddings + raw covariates
    Z_X = np.hstack([X_embed, X_raw])
    pipeline_zx = CausalDMLPipeline()
    ate_zx, _, _ = pipeline_zx.fit_predict(Y, T, Z_X)
    se_zx = pipeline_zx.ate_se

    # Delta: difference in ATE estimates
    delta = abs(ate_z - ate_zx)
    pooled_se = np.sqrt(se_z**2 + se_zx**2)
    ...
    if delta < 0.02:
        ...
        note = (
            "Adding raw covariates does not meaningfully change the ATE estimate. "
            "The embedding captures the relevant confounding information."
        )
    elif delta < 0.05:
        assessment = "APPROXIMATELY_SUFFICIENT"
        note = (
            "Adding raw covariates produces a small change in the ATE estimate. "
            "The embedding captures most confounding information, with minor residual."
        )
    else:
        assessment = "INSUFFICIENT"
        note = (
            "Adding raw covariates meaningfully changes the ATE estimate. "
            "The embedding does not fully capture confounding; the DML cross-fitting "
            "may be compensating for missing information in the embedding."
        )

    return {
        "ate_embedding_only": ate_z,
        "se_embedding_only": se_z,
        "ate_embedding_plus_raw": ate_zx,
        "se_embedding_plus_raw": se_zx,
        "delta": delta,
        "t_statistic": t_stat,
        "true_ate": true_ate,
        "bias_embedding_only": ate_z - true_ate,
        "bias_embedding_plus_raw": ate_zx - true_ate,
...
# =============================================================================
# VEITCH IMPROVEMENT 4: LINEAR REPRESENTATION PROBING
# =============================================================================
...
def probe_linear_representations(
    ...
    """Test the Linear Representation Hypothesis (Park et al., 2023; Veitch).

    Probes whether key causal variables (ability, treatment, outcome) are
    linearly decodable from the learned embeddings. This connects to
    Veitch's work on the geometry of representations in large models.

    Ideal causal embedding:
        - High R² for ability (captures confounders)
        - Low accuracy for treatment (~50%, chance level = no treatment leakage)
        - High R² for outcome (informative for prediction)

    Args:
        X_embed: Embedding array (n, d).
        ability: Latent ability array (n,).
        treatment: Binary treatment array (n,).
        outcome: Outcome array (n,).

    Returns:
        Dictionary with probing results for each variable.
    """
    ...
    # Probe 1: Ability (continuous) — R² via 5-fold CV
    reg_ability = LinearRegression()
    r2_ability_scores = cross_val_score(reg_ability, X_embed, ability, cv=5, scoring='r2')
    r2_ability = float(np.mean(r2_ability_scores))

    # Probe 2: Treatment (binary) — Accuracy via 5-fold CV
    clf_treatment = LogisticRegression(max_iter=1000)
    acc_treatment_scores = cross_val_score(clf_treatment, X_embed, treatment, cv=5, scoring='accuracy')
    acc_treatment = float(np.mean(acc_treatment_scores))

    # Probe 3: Outcome (continuous) — R² via 5-fold CV
    reg_outcome = LinearRegression()
    r2_outcome_scores = cross_val_score(reg_outcome, X_embed, outcome, cv=5, scoring='r2')
    r2_outcome = float(np.mean(r2_outcome_scores))

    # Assess the embedding quality
    treatment_leakage = acc_treatment - 0.5  # How much above chance
    is_treatment_neutral = acc_treatment < 0.6  # Within 10% of chance

    if r2_ability > 0.3 and is_treatment_neutral and r2_outcome > 0.3:
        profile = "CAUSALLY_IDEAL"
        note = (
            "The embedding captures ability (confounder) and outcome information "
            "while remaining approximately neutral to treatment assignment. "
            "This is the ideal profile for causal inference."
        )
    elif r2_ability > 0.3 and not is_treatment_neutral:
        profile = "CONFOUNDED"
        note = (
            "The embedding captures ability but also leaks treatment information. "
            "This may introduce bias if the DML does not fully adjust. "
            "Adversarial debiasing targets this issue."
        )
    elif r2_ability < 0.1:
        profile = "UNINFORMATIVE"
        note = (
            "The embedding does not capture ability (the main confounder). "
            "Causal estimates from this embedding may be unreliable."
        )
    else:
        profile = "MIXED"
        note = (
            "The embedding has moderate ability capture with some treatment leakage. "
            "The DML cross-fitting may partially compensate."
        )

    results = {
        "ability_r2": r2_ability,
        "ability_r2_std": float(np.std(r2_ability_scores)),
        "treatment_accuracy": acc_treatment,
        "treatment_accuracy_std": float(np.std(acc_treatment_scores)),
        "treatment_leakage": treatment_leakage,
        "outcome_r2": r2_outcome,
        "outcome_r2_std": float(np.std(r2_outcome_scores)),
        "profile": profile,
        "interpretation": note,
    }

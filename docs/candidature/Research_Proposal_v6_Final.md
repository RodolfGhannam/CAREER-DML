...
1.  **Predictive GRU:** A standard Gated Recurrent Unit (**Cho et al., 2014**) [17] trained to predict the outcome. This is our baseline, which relies entirely on the DML cross-fitting to adjust for confounding.
2.  **Causal GRU (VIB):** A Variational Information Bottleneck is added to the GRU to compress the representation while retaining information about both Y and T, following Alemi et al. (2017).
3.  **Debiased GRU (Adversarial):** An adversarial head is added to the GRU, which tries to predict the treatment from the embedding. The main model is trained to produce embeddings that are informative for Y but approximately independent of T.
4.  **Two-Stage Causal GRU (Veitch-faithful):** A faithful implementation of the two-stage procedure in Veitch et al. (2020): (1) pre-train the GRU on outcome prediction, then (2) freeze the encoder and train a VIB compression layer with dual prediction heads for both Y and T. This separates representation learning from causal compression.
...
A systematic sweep of the VIB compression parameter β ∈ {0.0001, 0.001, 0.01, 0.05, 0.1, 0.5, 1.0} reveals that the information bottleneck approach is sensitive to β across the entire range (38.9%–50.1% error), while the Predictive GRU (7.6%), Adversarial (18.4%), and Two-Stage (20.5%) approaches require no such tuning. A formal causal sufficiency test (Veitch et al., 2020) and linear representation probing analysis further confirm that the DML cross-fitting mechanism is the primary source of bias correction, rather than the embedding design itself — a finding with implications for the practical deployment of causal ML pipelines.

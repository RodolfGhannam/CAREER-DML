...
While our approach is rooted in the reduced-form tradition of causal ML, it builds an explicit bridge to the structural econometrics tradition of the labor market. The learned embedding `z` can be interpreted as a rich, non-parametric approximation of key latent variables in structural models:
...
In sum, this paper's contributions are fourfold. First, we introduce **CAREER-DML**, a robust and transparent framework for causal inference on career trajectories. Second, using this framework as a computational laboratory, we **document** the *Sequential Embedding Ordering Phenomenon*, an empirical finding that challenges the direct application of some causal representation learning methods to this domain. Third, we **characterize** the *Signal-to-Noise Frontier*, providing a rigorous justification for the use of large-scale administrative data. Finally, we **build** a *conceptual bridge to structural labor economics*, showing how our embeddings can serve as non-parametric analogs of classical latent variables.
...
## 3. Data Sources for Semi-Synthetic DGP
...
### 3.1. Note on AI Exposure Metrics and the Generative AI Shock

The reliance on the Felten et al. (2021) AI Occupational Exposure (AIOE) scores is a deliberate methodological choice to establish a robust, pre-generative AI baseline. The landscape of Artificial Intelligence has undergone a near-vertical growth trajectory since late 2022. Recent data illustrates this unprecedented shift: regular organizational use of Generative AI doubled from 33% in 2023 to 65% by early 2024 [13], while frequent AI usage among US employees tripled between Q2 2023 and Q4 2025 [14].
...
### 4.1. Gain Decomposition: From Heckman to Sequential Embeddings
...
[10] McKinsey & Company. (2025). *The state of AI in 2025: Agents, innovation, and transformation*.

[11] OECD. (2026, January). *AI use by individuals surges across the OECD as adoption by firms continues to expand*.

[12] OECD. (2024). *Job Creation and Local Economic Development 2024: Country Notes - Denmark*.

[13] McKinsey & Company. (2024). *The state of AI in early 2024: Gen AI adoption spikes and starts to generate value*.

[14] Gallup. (2026, January). *Frequent Use of AI in the Workplace Continued to Rise in Q4*.
